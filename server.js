...
// IMPORTANT: In a real application, use an environment variable for the secret!
const JWT_SECRET = 'your-super-secret-and-long-random-string-for-jwt';
const usersFilePath = path.join(__dirname, 'users.xlsx');
const loginAttemptsFilePath = path.join(__dirname, 'logindata.xlsx');
const detailsFilePath = path.join(__dirname, 'Infernos Details', 'User Details .xlsx');
const profilePicturesPath = path.join(__dirname, 'Profile Picture');
...
// ---
/**
 * Creates the users.xlsx file with credentials from 106 to 210 if it doesn't exist.
 */
function initializeUserStore() {
    if (fs.existsSync(usersFilePath)) {
        console.log('Users file (users.xlsx) already exists. Skipping creation.');
        return;
    }
...
function loadUsers() {
    try {
        if (fs.existsSync(usersFilePath)) {
            ...
            console.log(`Successfully loaded ${users.length} users from users.xlsx.`);

            // Check if the loaded data is in the old, insecure format.
            if (users.length > 0 && !users[0].hasOwnProperty('PasswordHash')) {
                console.error('\x1b[31m%s\x1b[0m', 'FATAL: The users.xlsx file appears to be in an old format with plaintext passwords.');
                console.error('\x1b[33m%s\x1b[0m', 'Please delete the "users.xlsx" file and restart the server to regenerate it with secure, hashed passwords.');
            ...
            if (fs.existsSync(detailsFilePath)) {
                ...
                const detailsWorkbook = XLSX.readFile(detailsFilePath);
                const detailsWorksheet = detailsWorkbook.Sheets[detailsWorkbook.SheetNames[0]];
                const detailsList = XLSX.utils.sheet_to_json(detailsWorksheet);
                ...
                console.log(`"${path.basename(detailsFilePath)}" not found. Using default data from users.xlsx.`);
            }
        } else {
            console.error('Users file (users.xlsx) not found. Please create it or restart the server to initialize it.');
        }
    } catch (error) {
    ...
    try {
        ...
        try {
            ...
            if (fs.existsSync(loginAttemptsFilePath)) {
                workbook = XLSX.readFile(loginAttemptsFilePath);
                const worksheet = workbook.Sheets[sheetName];
                XLSX.utils.sheet_add_json(worksheet, logData, {
                    header: ["Timestamp", "Login ID", "Status"],
                    skipHeader: true,
                    origin: -1
                });
            } else {
            ...
            XLSX.writeFile(workbook, loginAttemptsFilePath);
        } catch (error) {
...
app.listen(port, () => {
    ...
    initializeUserStore();
    // Load users into memory for fast access
    loadUsers();
